...
def main():
    mode = input("Режим поиска (tokens/lemmas): ").strip().lower()
    ...
    search_engine = VectorSearchEngine(TF_IDF_DIR, mode=mode)

    print("\nПоисковая система успешно проинициализирована")
    print("Введите поисковый запрос или введите 'q' для выхода:")
    ...
    while True:
        ...
        if not results:
            ...
            for idx, (filename, score) in enumerate(results, 1):
                print(f"{idx}. Сходство: {score:.4f}. Документ: {filename}")
