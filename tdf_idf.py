...
def process_document(doc_id, html_filename, token_idf, lemma_idf):
    ...
    print(f"TF-IDF по токенам и лемма рассчитан для документа с ID: {doc_id}")
...
def main():
    ...
    if total_docs == 0:
        print("Нет токенов в data/tokens/")
        return
    ...
    html_files = {f.split('-')[0]: f for f in os.listdir(PAGES_DIR) if f.endswith('.html')}
    ...
    for doc_id in doc_ids:
        ...
        if html_filename:
            ...
            print(f"Не найден HTML-файл для doc_id={doc_id}")
